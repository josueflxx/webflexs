...
def price_list_uses_product_prices(price_list):
    """The seeded ``base`` lists follow the current admin product price.

    Their legacy items are March 2026 snapshots, not negotiated overrides.
    Other lists keep their explicit per-product prices.
    """
    return bool(price_list and price_list.slug == "base")


def build_price_list_item_map(price_list, product_ids):
    if not price_list or not product_ids or price_list_uses_product_prices(price_list):
        return {}
...
def get_base_price_for_product(product, price_list=None, item_map=None):
    if price_list_uses_product_prices(price_list):
