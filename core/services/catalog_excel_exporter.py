...
from core.services.pricing import calculate_final_price, price_list_uses_product_prices
...
MEASUREMENTS_GUIDE_SHEET_TITLE = "ACERO"
MEASUREMENTS_GUIDE_IMAGE = ("core", "static", "core", "img", "todas_las_medidas.png")
MEASUREMENTS_GUIDE_MAX_WIDTH = 760
MEASUREMENTS_GUIDE_ANCHOR_MIN_COLUMN = 5
...
def _append_measurements_guide_image(worksheet):
    """Place the measurement guide beside the product table on the ACERO sheet."""
    if str(worksheet.title).strip().casefold() != MEASUREMENTS_GUIDE_SHEET_TITLE.casefold():
        return False
    ...
    image_path = os.path.join(settings.BASE_DIR, *MEASUREMENTS_GUIDE_IMAGE)
    if not os.path.exists(image_path):
        return False

    image = Image(image_path)
    if image.width > MEASUREMENTS_GUIDE_MAX_WIDTH:
        scale = MEASUREMENTS_GUIDE_MAX_WIDTH / image.width
        image.width = int(image.width * scale)
        image.height = int(image.height * scale)

    anchor_column = max(worksheet.max_column + 2, MEASUREMENTS_GUIDE_ANCHOR_MIN_COLUMN)
    worksheet.add_image(image, f"{get_column_letter(anchor_column)}1")
    return True
...
def _append_index_sheet(workbook, template, stats, generated_at):
    ...
    _prepare_worksheet(worksheet, "FF6B35")
    ...
    for col_idx, header in enumerate(headers, start=1):
        ...
        cell.font = INDEX_TABLE_HEADER_FONT
        cell.fill = INDEX_TABLE_HEADER_FILL
        cell.border = INDEX_TABLE_HEADER_BORDER
        if col_idx == 2:
            cell.alignment = Alignment(horizontal="right", vertical="center")
        elif col_idx == 3:
            cell.alignment = Alignment(horizontal="center", vertical="center")
    ...
    for sheet_name, row_count in rows_by_sheet.items():
        ...
        c1.font = Font(name="Segoe UI", bold=True, color="0F172A", size=10)
        c1.alignment = Alignment(horizontal="left", vertical="center", indent=1)
        
        # Styles for Cell 2 (Productos)
        c2 = worksheet.cell(row=row_idx, column=2)
        c2.font = Font(name="Segoe UI", color="475569", size=10)
        c2.alignment = Alignment(horizontal="right", vertical="center")
        c2.number_format = '#,##0'
        
        # Styles for Cell 3 (Vigencia)
        c3 = worksheet.cell(row=row_idx, column=3)
...
def build_catalog_workbook(template, price_list=None, discount_percentage=None):
    ...
    price_map = None
    if price_list and not price_list_uses_product_prices(price_list):
        price_map = {
            product_id: price
    ...
    for sheet_config in sheets:
        ...
        if _sheet_uses_clamp_measure_grouping(sheet_config):
            ...
            _append_measurements_guide_image(worksheet)
            continue
