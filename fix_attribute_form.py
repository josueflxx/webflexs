...
<div class="admin-form-container">
    ...
    <form method="post">
        ...
        <div class="form-group">
            <label for="slug" class="form-label">Slug (Código)</label>
            <input type="text" name="slug" id="slug" class="form-input" value="{{ form.instance.slug|default:'' }}">
            <small class="text-muted">Debe ser único dentro de la categoría. Solo letras minúsculas, números y
                guiones.</small>
            {% if form.slug.errors %}
            <div class="error-feedback">{{ form.slug.errors.0 }}</div>
            {% endif %}
        </div>
        ...
        <div class="form-group">
            ...
            <select name="type" id="type" class="form-select">
                ...
                <option value="{{ value }}" {% if form.instance.type == value %}selected{% endif %}>{{ label }}</option>
                {% endfor %}
            </select>
        </div>
        ...
        <div class="form-group checkbox-group">
            <label class="form-label" style="display: flex; align-items: center; gap: 10px; cursor: pointer;">
                <input type="checkbox" name="required" id="required" {% if form.instance.required %}checked{% endif %}>
                <span>Es obligatorio</span>
            </label>
            <small class="text-muted" style="display: block; margin-top: 5px;">Si se marca, este atributo será
                obligatorio al editar productos de esta categoría.</small>
        </div> 
...
<script>
    ...
    document.getElementById('name').addEventListener('input', function () {
        // Check if we are creating a new attribute (no instance slug yet or empty)
...
file_path = r'c:\Users\Brian\Desktop\webflexs\admin_panel\templates\admin_panel\categories\attribute_form.html'
