...
class CatalogClientExcelDownloadTests(TestCase):
    ...
    def test_approved_client_can_download_published_catalog_excel(self):
        ...
        self.assertEqual(workbook["INDICE"]["C7"].value, "Última Actualización")
        self.assertEqual(workbook["INDICE"]["A8"].value, "Categoria XLSX")
        self.assertEqual(workbook["INDICE"]["D8"].value, "➜ Ver Categoria XLSX")
...
class CatalogExcelGroupedExportTests(TestCase):
    ...
    def test_sheet_can_group_products_by_subcategory_blocks(self):
        ...
        grandchild = Category.objects.create(name="Con pestana", parent=child, order=3, is_active=True)
        greatgrandchild = Category.objects.create(name="Reforzado", parent=grandchild, order=4, is_active=True)
        ...
        greatgrandchild_product = Product.objects.create(
            sku="GREAT-001",
            name="Producto tercer nivel",
            price=Decimal("400.00"),
            stock=4,
            is_active=True,
            category=greatgrandchild,
        )
        greatgrandchild_product.categories.add(greatgrandchild)
        ...
        self.assertTrue(str(worksheet["A1"].fill.fgColor.rgb).endswith("1E293B"))
        ...
        self.assertTrue(str(worksheet["A5"].fill.fgColor.rgb).endswith("FFF7ED"))
        ...
        self.assertTrue(str(worksheet["A9"].fill.fgColor.rgb).endswith("F8FAFC"))
        ...
        self.assertEqual(
            worksheet["A13"].value,
            "Subcategoria nivel 3: Bujes > Con pestana > Reforzado (1 productos)",
        )
        self.assertTrue(str(worksheet["A13"].fill.fgColor.rgb).endswith("F1F5F9"))
