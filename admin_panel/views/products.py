...
def category_list(request):
    ...
    brand_candidates = detect_category_brand_candidates(categories=all_categories)
    brand_candidate_map = {candidate.category.pk: candidate for candidate in brand_candidates}
    for row in tree_rows:
        row['brand_candidate'] = brand_candidate_map.get(row['category'].pk)
    ...
    return render(request, 'admin_panel/categories/list.html', {
        ...
        'brand_integration_metrics': category_brand_integration_metrics(brand_candidates),
    })
...
def products_uncategorized(request):
    ...
    page_size_raw = request.GET.get('per_page', '').strip()
    per_page = int(page_size_raw) if page_size_raw.isdigit() and int(page_size_raw) in page_size_options else 50
    ...
    products = products.select_related('supplier_ref').order_by('name').distinct()
    paginator = Paginator(products, per_page)
    page_obj = paginator.get_page(request.GET.get('page'))
    page_products = list(page_obj.object_list)
    
    # Smart suggestions are calculated only for the visible page. Previously
    # this loop processed every uncategorized product before rendering.
...
def category_manage_products(request, pk):
    ...
    if request.method == 'POST':
        ...
        if action == "convert_blocks":
            conversions = None
            raw_conversions = str(request.POST.get("conversions_json") or "").strip()
            if raw_conversions:
                try:
                    conversions = json.loads(raw_conversions)
                except json.JSONDecodeError:
                    messages.error(request, "La seleccion de bloques no es valida. Abri el modal e intenta nuevamente.")
                    return redirect('admin_category_products', pk=pk)
                if not isinstance(conversions, list) or not all(isinstance(item, dict) for item in conversions):
                    messages.error(request, "La seleccion de bloques no es valida. Abri el modal e intenta nuevamente.")
                    return redirect('admin_category_products', pk=pk)
                conversions = [
                    {
                        "original": _clean_order_block_label(item.get("original")),
                        "new_name": _clean_order_block_label(item.get("new_name")),
                    }
                    for item in conversions
                    if _clean_order_block_label(item.get("original"))
                    and _clean_order_block_label(item.get("new_name"))
                ]
                if not conversions:
                    messages.warning(request, "Selecciona al menos un bloque con un nombre valido para convertir.")
                    return redirect('admin_category_products', pk=pk)

            result = convert_category_blocks_to_subcategories(category, conversions=conversions)
...
def product_grid_add_brand_association(request):
    ...
    product_id = data.get('product_id')
    expected_product_sku = str(data.get('product_sku') or '').strip()
    rubro_id_str = data.get('rubro_id')
    subrubro_id_str = data.get('subrubro_id')
    ...
    product = get_object_or_404(Product, pk=product_id)
    if expected_product_sku and product.sku != expected_product_sku:
        return JsonResponse({
            'status': 'error',
            'message': 'El producto cambió desde que abriste esta página. Recargala antes de asociar la marca.'
        }, status=409)
