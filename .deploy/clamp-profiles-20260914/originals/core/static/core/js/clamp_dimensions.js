...
export function attachClampDimensions(viewer, { variant = 'curva', controls, footer }) {
    const assembly = variant === 'assembly';
    const half = assembly ? 0.045 : 0.06635;
    const rod = 0.00635;
    const ceiling = assembly ? 0.25365 : 0.18;
    const threadTop = assembly ? 0.079 : 0.061;
    const z = 0.008;
    ...
    const definitions = [
        ...
        ['c', 'C', 'Largo útil', 'Desde la cara interior del arco hasta el extremo de las patas.'],
    ...
    const witnesses = [['c0', 'c2'], ['c1', 'c3'], ['d0', 'd2'], ['d1', 'd3'], ['a1', 'al']].map(([from, to]) => {
        const line = document.createElementNS(NS, 'line');
    ...
    function draw() {
        ...
        for (const { name, line, label } of dimensions) {
            ...
            label.hidden = !valid;
            if (!valid) continue;
            const point = name === 'a' ? coordinates('al') : { x: (start.x + end.x) / 2, y: (start.y + end.y) / 2 };
            if (!point) continue;
            let x = point.x;
            let y = point.y;
            if (name === 'b') y -= 19;
