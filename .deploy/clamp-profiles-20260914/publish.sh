...
STAGE=/var/tmp/webflexs-clamp-profiles-20260914
BACKUP=/var/backups/webflexs/clamp-profiles-20260914-r2
...
rollback() {
    ...
    printf '%s\n' CLAMP_PROFILES_DEPLOY_FAILED_PREVIOUS_FILES_RESTORED
    exit 1
}
trap rollback ERR
for file in "${FILES[@]}"; do
    install -d -m 755 "$(dirname "$APP/$file")"
    install -o root -g www-data -m 644 "$STAGE/package/$file" "$APP/$file.clamp-profiles.tmp"
    mv -f "$APP/$file.clamp-profiles.tmp" "$APP/$file"
done
"$PYTHON" manage.py check --settings=flexs_project.settings.production
"$PYTHON" manage.py collectstatic --noinput --settings=flexs_project.settings.production --verbosity 0
for asset in core/js/home_clamp.js core/js/clamp_viewer.js core/js/clamp_dimensions.js core/css/home_clamp.css core/models/abrazadera-curva-plaqueta-tuercas.glb core/models/abrazadera-semicurva-plaqueta-tuercas.glb; do
...
for attempt in {1..10}; do
    if curl --connect-timeout 3 --max-time 8 --fail --silent -o "$BACKUP/public-home.html" https://flexsrepuestos.shop/ && grep -q 'home-clamp-profiles' "$BACKUP/public-home.html"; then ready=true; break; fi
    sleep 1
done
test "$ready" = true
sha256sum -c "$STAGE/package.sha256"
trap - ERR
printf '%s\n' CLAMP_PROFILES_DEPLOY_COMPLETED
