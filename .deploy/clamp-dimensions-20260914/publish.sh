...
STAGE=/var/tmp/webflexs-clamp-dimensions-20260914
BACKUP=/var/backups/webflexs/clamp-dimensions-20260914
...
rollback() {
    ...
    touch "$APP/core/static/core/js/home_clamp.js"
    "$PYTHON" manage.py collectstatic --noinput --settings=flexs_project.settings.production --verbosity 0
    systemctl restart gunicorn
    printf '%s\n' CLAMP_DIMENSIONS_DEPLOY_FAILED_PREVIOUS_FILES_RESTORED
    exit 1
}
trap rollback ERR
for file in "${FILES[@]}"; do
    install -d -m 755 "$(dirname "$APP/$file")"
    install -o root -g www-data -m 644 "$STAGE/package/$file" "$APP/$file.clamp-dimensions.tmp"
    mv -f "$APP/$file.clamp-dimensions.tmp" "$APP/$file"
done
"$PYTHON" manage.py check --settings=flexs_project.settings.production
"$PYTHON" manage.py collectstatic --noinput --settings=flexs_project.settings.production --verbosity 0
for asset in core/js/home_clamp.js core/js/clamp_viewer.js core/js/clamp_dimensions.js core/css/clamp_viewer.css core/css/clamp_dimensions.css core/models/abrazadera-plaqueta-tuercas.glb; do
...
for attempt in {1..10}; do
    if curl --connect-timeout 3 --max-time 8 --fail --silent -o "$BACKUP/public-home.html" https://flexsrepuestos.shop/ && grep -q 'abrazadera-plaqueta-tuercas.glb' "$BACKUP/public-home.html"; then ready=true; break; fi
    sleep 1
done
test "$ready" = true
sha256sum -c "$STAGE/package.sha256"
trap - ERR
printf '%s\n' CLAMP_DIMENSIONS_DEPLOY_COMPLETED
