(function () {
    ...
    function restoreFiltersFromUrl() {
        ...
        if (elements.category) elements.category.value = params.get("category_id") || "";
        if (elements.supplier) elements.supplier.value = params.get("supplier") || "";
        if (elements.status && params.has("status")) {
            elements.status.value = params.get("status") || "all";
        }
        if (elements.stock && params.has("stock")) {
            elements.stock.value = params.get("stock") || "all";
        }
        if (elements.assignment && params.has("assignment")) {
            elements.assignment.value = params.get("assignment") || "available";
        }
    }
    ...
    function syncFiltersToUrl() {
        ...
        const params = new URLSearchParams(window.location.search);
        managedUrlFilters.forEach((key) => params.delete(key));
        ...
        window.history.replaceState({ brandWorkspaceFilters: true }, "", nextUrl);
    }
