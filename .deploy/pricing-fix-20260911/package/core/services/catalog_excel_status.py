...
def latest_catalog_excel_source_change(template):
    ...
    from catalog.models import Category, CategoryProductOrder, PriceList, PriceListItem, Product
    ...
    pricing_path = os.path.join(settings.BASE_DIR, 'core', 'services', 'pricing.py')
    ...
    design_mtimes = []
    for path in (exporter_path, pricing_path, measurements_guide_path):
    ...
    timestamps = [
        ...
        PriceList.objects.aggregate(value=Max("updated_at")).get("value"),
        PriceListItem.objects.aggregate(value=Max("updated_at")).get("value"),
