...
try{
 ...
 await context.route('**/*',r=>r.request().url().startsWith('http://127.0.0.1:8776')?r.continue():r.abort());
 for(const [name,path] of [['home','/'],['catalog','/catalogo/abrazaderas-a-medida/']]){
  ...
  await page.goto('http://127.0.0.1:8776'+path,{waitUntil:'domcontentloaded',timeout:90000});
  ...
  for(const profile of ['plana','curva','semicurva']){
   ...
   await page.locator(name==='home'?'#homeClamp':'.clamp-simulator-card').screenshot({path:`output/clamp-profiles-review/${name}-${profile}.png`});
  }
  ...
  await page.locator(name==='home'?'#homeClamp':'.clamp-simulator-card').screenshot({path:`output/clamp-profiles-review/${name}-mobile.png`});
