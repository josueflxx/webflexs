...
pages={path:get_template(template).template.render(Context(context)).encode() for path,template in [('/','core/home.html'),('/catalogo/abrazaderas-a-medida/','catalog/clamp_request.html')]}
class Handler(SimpleHTTPRequestHandler):
 def do_GET(self):
  path=urlsplit(self.path).path
  if path in pages:
   self.send_response(200);self.send_header('Content-Type','text/html; charset=utf-8');self.end_headers();self.wfile.write(pages[path]);return
 ...
 def translate_path(self,path):
  ...
  if '..' in Path(name).parts:return str(PROJECT/'output/clamp-dimensions-review/missing')
  return finders.find(name) or str(PROJECT/'output/clamp-dimensions-review/missing')
print('DIMENSIONS PREVIEW http://127.0.0.1:8775',flush=True)
